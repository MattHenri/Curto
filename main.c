...
#include "jogadores/jogador_16839482.h"
...
void informar_maos_para_jogadores(int rodada, const Rodada* r) {
    nova_rodada_jogador_16839482(rodada, r->carta_virada, r->cartas_por_jogador, r->maos[0]);
...
void coletar_apostas(Rodada* r) {
    ...
    for (int i = 0; i < NUM_JOGADORES; i++) {
        ...
        if (j == 0) r->apostas[j] = apostar_jogador_16839482(r->apostas);
...
int processar_jogadas(Rodada* r, Jogada* jogadas) {
    ...
    for (int i = 0; i < jogo.num_jogadores; i++) {
        ...
        if (j == 0) idx = jogar_jogador_16839482(NULL, 0);
